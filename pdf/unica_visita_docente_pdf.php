...
FROM 
tbl_personas p, tbl_empresas_practica ep, tbl_funciones_practica fp, tbl_comentarios_alumnos ca, tbl_desempeno_practica dp, tbl_evaluaciones_practica evp where p.id_persona='$id_persona' AND ep.id_persona='$id_persona' AND fp.id_persona='$id_persona' AND ca.id_persona='$id_persona' and dp.id_persona='$id_persona' and evp.id_persona='$id_persona' AND p.id_persona=ep.id_persona AND fp.numero_visita='Única Supervisión' and ca.numero_visita='Única Supervisión' AND dp.numero_visita='Única Supervisión' AND evp.numero_visita='Única Supervisión'";
...
$resultado = mysqli_query($connection, $sql);
	...
	if(empty($row) || $row['labora_dentro']=='NO')
		{
			echo '<script>alert("La supervisión para este alumno aún no ha sido generada ó no labora dentro de la empresa.");window.location.href="../vistas/supervisiones_realizadas_vista.php";</script>';
		}
		else
		{
	...
	$pdf->ln(5);
    $pdf->Cell(0,5,utf8_decode('ÚNICA SUPERVISIÓN'),0,1,'C');
    $pdf->ln(5);
    ...
    $pdf->ln(2);
	$pdf->SetX(20);
	//$pdf->multicell(170,5,utf8_decode('Nombre del puesto que desempe�a el estudiante: '.$row[''].''),0);
	$pdf->SetX(20);
    $pdf->ln(10);
	...
	$pdf->ln(2);
	$pdf->SetX(20);
	$pdf->multicell(170,5,utf8_decode('¿El estudiante tendrá alguna oportunidad de empleo en la organización?: '.$row['oportunidad_empleo'].' '),0);
	$pdf->SetX(20);
    $pdf->ln(2);
	$pdf->ln(2);
	$pdf->SetX(20);

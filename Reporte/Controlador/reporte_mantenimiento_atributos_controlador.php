...
class myPDF extends FPDF
{
    function header()
    {
        ...
        $this->Cell(330, 10, utf8_decode("REPORTE DE MANTENIMIENTO ATRIBUTOS"), 0, 0, 'C');
        $this->ln(17);
        $this->SetFont('Arial', '', 12);
        $this->Cell(60, 10, utf8_decode("ATRIBUTOS EXISTENTES"), 0, 0, 'C');
    ...
    function headerTable()
    {
        ...
        $this->Cell(90, 7, "ATRIBUTO", 1, 0, 'C');
        $this->Cell(30, 7, "REQUERIDO", 1, 0, 'C');
        $this->Cell(60, 7, "TIPO PERSONA", 1, 0, 'C');
    ...
    function viewTable()
    {
        ...
        $sql = "select  atributo, requerido, tipo_persona
               FROM tbl_atributos a
               INNER JOIN tbl_tipos_persona tp ON a.id_tipo_persona=tp.id_tipo_persona";
        $stmt = $instancia_conexion->ejecutarConsulta($sql);

        while ($reg = $stmt->fetch_object()) {

            $this->SetFont('Times', '', 12);
            $this->Cell(90, 7, utf8_decode($reg->atributo), 1, 0, 'C');
            $this->Cell(30, 7, $reg->requerido, 1, 0, 'C');
            $this->Cell(60, 7, $reg->tipo_persona, 1, 0, 'C');

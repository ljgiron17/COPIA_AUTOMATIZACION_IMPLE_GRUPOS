...
class myPDF extends FPDF
{
    function header()
    {
        ...
        $this->Cell(330, 10, utf8_decode("REPORTE DE MANTENIMIENTO DEPARTAMENTOS"), 0, 0, 'C');
        $this->ln(17);
        $this->SetFont('Arial', '', 12);
        $this->Cell(70, 10, utf8_decode("DEPARTAMENTOS EXISTENTES"), 0, 0, 'C');
    ...
    function headerTable()
    {
        ...
        $this->Cell(90, 7, utf8_decode("DEPARTAMENTO"), 1, 0, 'C');
    ...
    function viewTable()
    {
        ...
        $sql = " select departamento FROM tbl_departamentos";
        $stmt = $instancia_conexion->ejecutarConsulta($sql);

        while ($reg = $stmt->fetch_array(MYSQLI_ASSOC)) {

            $this->SetFont('Times', '', 12);
            $this->Cell(90, 7, utf8_decode($reg['departamento']), 1, 0, 'C');

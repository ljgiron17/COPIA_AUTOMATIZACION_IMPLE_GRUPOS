...
class myPDF extends FPDF
{
    function header()
    {
        ...
        $this->Cell(330, 10, utf8_decode("REPORTE DE MANTENIMIENTO MUNICIPIOS"), 0, 0, 'C');
        $this->ln(17);
        $this->SetFont('Arial', '', 12);
        $this->Cell(70, 10, utf8_decode("MUNICIPIOS EXISTENTES"), 0, 0, 'C');
    ...
    function headerTable()
    {
        ...
        $this->Cell(80, 7, utf8_decode("MUNICIPIO"), 1, 0, 'C');
        $this->Cell(70, 7, utf8_decode("CÓDIGO"), 1, 0, 'C');
        $this->Cell(80, 7, utf8_decode("DEPARTAMENTO"), 1, 0, 'C');
    ...
    function viewTable()
    {
        ...
        $sql = " select  municipio, codigo, departamento 
FROM tbl_municipios_hn m
INNER JOIN tbl_departamentos dp ON dp.id_departamento=m.id_departamento";
        $stmt = $instancia_conexion->ejecutarConsulta($sql);

        while ($reg = $stmt->fetch_array(MYSQLI_ASSOC)) {

            $this->SetFont('Times', '', 12);
            $this->Cell(80, 7, utf8_decode($reg['municipio']), 1, 0, 'C');
            $this->Cell(70, 7, utf8_decode($reg['codigo']), 1, 0, 'C');
            $this->Cell(80, 7, utf8_decode($reg['departamento']), 1, 0, 'C');

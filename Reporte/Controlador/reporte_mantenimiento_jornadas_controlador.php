...
class myPDF extends FPDF
{
    function header()
    {
        ...
        $this->Cell(330, 10, utf8_decode("REPORTE DE MANTENIMIENTO JORNADAS DOCENTES"), 0, 0, 'C');
        $this->ln(17);
        $this->SetFont('Arial', '', 12);
        $this->Cell(50, 10, utf8_decode("JORNADAS EXISTENTES"), 0, 0, 'C');
    ...
    function headerTable()
    {
        ...
        $this->Cell(50, 7, utf8_decode("JORNADA"), 1, 0, 'C');
    ...
    function viewTable()
    {
        ...
        $sql = "select jornada, descripcion FROM tbl_jornadas";
        $stmt = $instancia_conexion->ejecutarConsulta($sql);

        while ($reg = $stmt->fetch_array(MYSQLI_ASSOC)) {

            $this->SetFont('Times', '', 12);
            $this->Cell(50, 7, utf8_decode($reg['jornada']), 1, 0, 'C');

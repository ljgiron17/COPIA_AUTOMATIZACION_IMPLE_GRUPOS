...
class myPDF extends FPDF
{
    function header()
    {
        ...
        $this->Cell(330, 10, utf8_decode("REPORTE DE MANTENIMIENTO HORARIOS"), 0, 0, 'C');
        $this->ln(17);
        $this->SetFont('Arial', '', 12);
        $this->Cell(50, 10, utf8_decode("HORARIOS EXISTENTES"), 0, 0, 'C');
    ...
    function headerTable()
    {
        ...
        $this->Cell(30, 7, utf8_decode("HORAS"), 1, 0, 'C');
    ...
    function viewTable()
    {
        ...
        $sql = "select hora FROM tbl_hora";
        $stmt = $instancia_conexion->ejecutarConsulta($sql);

        while ($reg = $stmt->fetch_array(MYSQLI_ASSOC)) {

            $this->SetFont('Times', '', 12);
            $this->Cell(30, 7, $reg['hora'], 1, 0, 'C');

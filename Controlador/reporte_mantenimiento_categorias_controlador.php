...
class myPDF extends FPDF
{
    function header()
    {
        ...
        $this->Cell(330, 10, utf8_decode("REPORTE DE MANTENIMIENTO CATEGORIAS DOCENTES"), 0, 0, 'C');
        $this->ln(17);
        $this->SetFont('Arial', '', 12);
        $this->Cell(60, 10, utf8_decode("CATEGORIAS EXISTENTES"), 0, 0, 'C');
    ...
    function headerTable()
    {
        ...
        $this->Cell(30, 7, "CATEGORIA", 1, 0, 'C');
    ...
    function viewTable()
    {
        ...
        $sql = "select categoria, descripcion FROM tbl_categorias";
        $stmt = $instancia_conexion->ejecutarConsulta($sql);

        while ($reg = $stmt->fetch_array(MYSQLI_ASSOC)) {

            $this->SetFont('Times', '', 12);
            $this->Cell(30, 7,utf8_decode($reg['categoria']), 1, 0, 'C');

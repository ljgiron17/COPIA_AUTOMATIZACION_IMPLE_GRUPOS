...
class myPDF extends FPDF
{
    function header()
    {
        ...
        $this->Cell(330, 10, utf8_decode("REPORTE DE MANTENIMIENTO AULAS"), 0, 0, 'C');
        $this->ln(17);
        $this->SetFont('Arial', '', 12);
        $this->Cell(60, 10, utf8_decode("AULAS EXISTENTES"), 0, 0, 'C');
    ...
    function headerTable()
    {
        ...
        $this->Cell(30, 7, "ID AULA", 1, 0, 'C');
        $this->Cell(30, 7, utf8_decode("CÓDIGO"), 1, 0, 'C');
        $this->Cell(40, 7, utf8_decode("DESCRIPCIÓN"), 1, 0, 'C');
        $this->Cell(50, 7, "NOMBRE EDIFICIO", 1, 0, 'C');
        $this->Cell(70, 7, "TIPO DE AULA", 1, 0, 'C');
    ...
    function viewTable()
    {
        ...
        $sql = "select id_aula, au.codigo, descripcion, capacidad, ed.nombre, ta.tipo_aula
FROM tbl_aula au
INNER JOIN tbl_edificios ed ON au.id_edificio= ed.id_edificio
INNER JOIN tbl_tipo_aula ta ON  au.id_tipo_aula=ta.id_tipo_aula";
        $stmt = $instancia_conexion->ejecutarConsulta($sql);

        while ($reg = $stmt->fetch_array(MYSQLI_ASSOC)) {

            $this->SetFont('Times', '', 12);
            $this->Cell(30, 7, $reg['id_aula'], 1, 0, 'C');
            $this->Cell(30, 7, utf8_decode($reg['codigo']), 1, 0, 'C');
            $this->Cell(40, 7, utf8_decode($reg['descripcion']), 1, 0, 'C');
            $this->Cell(50, 7, $reg['nombre'], 1, 0, 'C');
            $this->Cell(70, 7, $reg['tipo_aula'], 1, 0, 'C');

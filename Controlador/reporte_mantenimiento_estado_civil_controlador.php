...
class myPDF extends FPDF
{
    function header()
    {
        ...
        $this->Cell(330, 10, utf8_decode("REPORTE DE MANTENIMIENTO ESTADO CIVIL"), 0, 0, 'C');
        $this->ln(17);
        $this->SetFont('Arial', '', 12);
        $this->Cell(60, 10, utf8_decode("ESTADO CIVIL EXISTENTES"), 0, 0, 'C');
    ...
    function headerTable()
    {
        ...
        $this->Cell(60, 7, utf8_decode("ESTADO CIVIL"), 1, 0, 'C');
        $this->Cell(50, 7, utf8_decode("DESCRIPCIÓN"), 1, 0, 'C');
    ...
    function viewTable()
    {
        ...
        $sql = "select estado_civil, descripcion FROM tbl_estadocivil";
        $stmt = $instancia_conexion->ejecutarConsulta($sql);

        while ($reg = $stmt->fetch_array(MYSQLI_ASSOC)) {

            $this->SetFont('Times', '', 12);
            $this->Cell(60, 7, utf8_decode($reg['estado_civil']), 1, 0, 'C');

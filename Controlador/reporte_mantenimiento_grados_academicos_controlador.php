...
class myPDF extends FPDF
{
    function header()
    {
        ...
        $this->Cell(330, 10, utf8_decode("REPORTE DE MANTENIMIENTO GRADOS ACADÉMICOS"), 0, 0, 'C');
        $this->ln(17);
        $this->SetFont('Arial', '', 12);
        $this->Cell(75, 10, utf8_decode("GRADOS ACADÉMICOS EXISTENTES"), 0, 0, 'C');
    ...
    function headerTable()
    {
        ...
        $this->Cell(60, 7, utf8_decode("GRADO ACADÉMICO"), 1, 0, 'C');
        $this->Cell(60, 7, utf8_decode("DESCRIPCIÓN"), 1, 0, 'C');
    ...
    function viewTable()
    {
        ...
        $sql = "select grado_academico, descripcion FROM tbl_grados_academicos";
        $stmt = $instancia_conexion->ejecutarConsulta($sql);

        while ($reg = $stmt->fetch_array(MYSQLI_ASSOC)) {

            $this->SetFont('Times', '', 12);
            $this->Cell(60, 7, utf8_decode($reg['grado_academico']), 1, 0, 'C');
            $this->Cell(60, 7, utf8_decode($reg['descripcion']), 1, 0, 'C');

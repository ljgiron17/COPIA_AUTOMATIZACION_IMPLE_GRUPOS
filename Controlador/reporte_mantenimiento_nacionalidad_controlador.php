...
class myPDF extends FPDF
{
    function header()
    {
        ...
        $this->Cell(330, 10, utf8_decode("REPORTE DE MANTENIMIENTO NACIONALIDAD"), 0, 0, 'C');
        $this->ln(17);
        $this->SetFont('Arial', '', 12);
        $this->Cell(70, 10, utf8_decode("NACIONALIDADES EXISTENTES"), 0, 0, 'C');
    ...
    function headerTable()
    {
        ...
        $this->Cell(50, 7, utf8_decode("NACIONALIDAD"), 1, 0, 'C');
        $this->Cell(50, 7, utf8_decode("PAIS"), 1, 0, 'C');
        $this->ln();
    }
    function viewTable()
    {
        global $instancia_conexion;
        $sql = "select nacionalidad, PAIS_NAC FROM tbl_nacionalidad";
        $stmt = $instancia_conexion->ejecutarConsulta($sql);

        while ($reg = $stmt->fetch_array(MYSQLI_ASSOC)) {

            $this->SetFont('Times', '', 12);
            $this->Cell(50, 7, utf8_decode($reg['nacionalidad']), 1, 0, 'C');
            $this->Cell(50, 7, utf8_decode($reg['PAIS_NAC']), 1, 0, 'C');
